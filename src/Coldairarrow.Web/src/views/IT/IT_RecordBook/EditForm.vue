  ...
  >
    <a-spin :spinning="loading">
      <a-form-model ref="form" :model="entity" :rules="rules" v-bind="layout">
        <a-form-model-item label="相关单号" prop="RefCode">
          <a-input v-model="entity.RefCode" autocomplete="off" />
        </a-form-model-item>
        <a-form-model-item label="台账类型" prop="Type">
...
export default {
  ...
  methods: {
    ...
    openForm(id, title) {
      ...
      if (id) {
        this.loading = true
        this.$http.post('/IT/IT_RecordBook/GetTheData', { id: id }).then(resJson => {
          this.loading = false
    ...
    handleSubmit() {
      this.$refs['form'].validate(valid => {
        ...
        this.loading = true
        this.$http.post('/IT/IT_RecordBook/SaveData', this.entity).then(resJson => {
          this.loading = false
