  ...
  >
    <a-spin :spinning="loading">
      <a-form-model ref="form" :model="entity" :rules="rules" v-bind="layout">
        ...
        <a-form-model-item label="托盘号ID" prop="TrayId">
          <a-input v-model="entity.TrayId" autocomplete="off" />
        </a-form-model-item>
        <a-form-model-item label="托盘分区ID" prop="ZonedId">
          <a-input v-model="entity.ZonedId" autocomplete="off" />
        </a-form-model-item>
        ...
        <a-form-model-item label="入库时间" prop="InputTime">
...
export default {
  ...
  methods: {
    ...
    openForm(id, title) {
      ...
      if (id) {
        this.loading = true
        this.$http.post('/IT/IT_LocalDetail/GetTheData', { id: id }).then(resJson => {
          this.loading = false
    ...
    handleSubmit() {
      this.$refs['form'].validate(valid => {
        ...
        this.loading = true
        this.$http.post('/IT/IT_LocalDetail/SaveData', this.entity).then(resJson => {
          this.loading = false
