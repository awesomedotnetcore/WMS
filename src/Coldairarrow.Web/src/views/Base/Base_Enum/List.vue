    ...
    <a-table ref="table" :columns="columns" :rowKey="row => row.Id" :dataSource="data" :pagination="pagination" :loading="loading" @change="handleTableChange" :rowSelection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }" :bordered="true" size="small">
      <span slot="action" slot-scope="text, record">
        <template>
          ...
          <a-divider v-if="!record.IsSystem" type="vertical" />
          <a @click="showEnumItem(record)">字典值</a>
        </template>
      </span>
    </a-table>

    <edit-form ref="editForm" :parentObj="this"></edit-form>
    <enum-item ref="enumItem"></enum-item>
  </a-card>
</template>

<script>
import EditForm from './EditForm'
import EnumItem from '../Base_EnumItem/List'
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.loading = true
      this.$http
        .post('/Base/Base_Enum/GetDataList', {
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise((resolve, reject) => {
            thisObj.$http.post('/Base/Base_Enum/DeleteData', ids).then(resJson => {
              resolve()
