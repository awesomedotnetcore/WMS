...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.loading = true
      this.$http
        .post('/PB/PB_MaterialPoint/GetDataList', {
    ...
    hanldleAdd() {
      ...
      //this.$refs.editForm.openForm(this.filters.PointId, '添加关联物料')
    },
    // handleEdit(id) {
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise((resolve, reject) => {
            thisObj.$http.post('/PB/PB_MaterialPoint/DeleteData?PointId=' + thisObj.filters.PointId, thisObj.ids)
              .then(resJson => {
                resolve()
                if (resJson.Success) {
    ...
    handleChoose(chooseRows){
      ...
      chooseRows.forEach( row =>{    
        ...
        this.loading = true
        this.$http.post('/PB/PB_MaterialPoint/SaveDatas', {id:this.filters.PointId,keys:this.targetKeys}).then(resJson => {
          this.loading = false
          if (resJson.Success) {
