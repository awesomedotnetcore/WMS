...
const columns = [
  { title: '托盘名称', dataIndex: 'Name', width: '20%' },
  { title: '托盘编码', dataIndex: 'Code', width: '20%' },
  { title: '托盘类型', dataIndex: 'PB_TrayType.Name', width: '20%' },
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      this.$http.post('/TD/TD_OutStorage/OutBlankTray', reqData)
        .then(resJson => {
          this.loading = false
          if (resJson.Success) {
