<template>
  <a-card title="发货管理">
    ...
    <a-drawer title="发货查询" :closable="true" :height="490" :visible="visible" placement="top" @close="e=>{this.visible=false}">
      <a-form-model layout="horizontal" :model="queryData.Search">
        <a-form-model-item>
          <a-input v-model="queryData.Search.Keyword" placeholder="发货单号"></a-input>
        </a-form-model-item>
        <a-form-model-item>
          <enum-select code="SendType" v-model="queryData.Search.Type"></enum-select>
        </a-form-model-item>
        <a-form-model-item>
          <a-date-picker placeholder="发货日期(开始)" :style="{width:'100%'}" :inputReadOnly="true" v-model="queryData.Search.OrderTimeStart" />
        </a-form-model-item>
        <a-form-model-item>
          <a-date-picker placeholder="发货日期(结束)" :style="{width:'100%'}" :inputReadOnly="true" v-model="queryData.Search.OrderTimeEnd" />
        </a-form-model-item>
        <a-form-model-item>
    ...
    <a-list :data-source="listData" :rowKey="item=>item.Id" item-layout="horizontal" :loading="loading" :pagination="pagination">
      <a-list-item slot="renderItem" slot-scope="item">
        <a-list-item-meta>
          ...
          <span slot="description">时间：{{ moment(item.SendTime).format('YY/M/D H:m') }} 数量：{{ item.TotalNum }}</span>
        </a-list-item-meta>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      SendSvc.GetDataList(this.queryData).then(resJson => {
        this.loading = false
        if (resJson.Success) {
    ...
    handlerShow(item) {
      this.$router.push({ path: `/OutStorage/SendDetail/${item.Id}` })
    },
