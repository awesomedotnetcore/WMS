<template>
  <a-card title="发货明细" :loading="loading">
    <div slot="extra">
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.loading = true
      SendSvc.GetTheData(this.id).then(resJson => {
        this.loading = false
        if (resJson.Success) {
    ...
    handleAudit(type) {
      this.loading = true
      SendSvc.Approval(this.id, type).then(resJson => {
        this.loading = false
        if (resJson.Success) {
    ...
    handleDel() {
      this.loading = true
      SendSvc.DeleteData(this.id).then(resJson => {
        this.loading = false
        if (resJson.Success) {
          this.$message.success(resJson.Msg)
          this.$router.go(-1)
